...
let private mkRide
    ...
    { Payment =
      ...
      Route = { RideDistance = Distance(float distance); Stops = [||] }
      Times =
...
let ``totalEarned sums net of every Earned line incl negative commission`` () =
    let rides =
        [ mkRide standardEarned 10m (utc (2024, 1, 1, 10)) "in_app" "card"
          mkRide standardEarned 10m (utc (2024, 1, 1, 11)) "in_app" "card" ]
...
let ``totalTips sums only Napiwek lines`` () =
    let rides =
        [ mkRide standardEarned 10m (utc (2024, 1, 1, 10)) "in_app" "card"
          mkRide [ "Prowizja Bolt", -5m; "Kurs", 25m ] 10m (utc (2024, 1, 1, 11)) "in_app" "card" ]
...
let ``medianEarnedPerRide even count averages two middles`` () =
    let mk net =
        mkRide [ "Kurs", net ] 1m (utc (2024, 1, 1, 10)) "in_app" "card"
...
let ``earnedPerActiveHour divides total earned by active hours`` () =
    let rides =
        [ mkRide [ "Kurs", 60m ] 10m (utc (2024, 1, 1, 10)) "in_app" "card" ]
...
let ``utilization is ride duration over active time`` () =
    ...
    let rides =
        [ mkRide [ "Kurs", 1m ] 1m (utc (2024, 1, 1, 10)) "in_app" "card"
          mkRide [ "Kurs", 1m ] 1m (utc (2024, 1, 1, 11)) "in_app" "card" ]
...
let ``totalDistance and earnedPerKm`` () =
    let rides =
        [ mkRide [ "Kurs", 20m ] 4m (utc (2024, 1, 1, 10)) "in_app" "card"
          mkRide [ "Kurs", 20m ] 6m (utc (2024, 1, 1, 11)) "in_app" "card" ]

    Assert.Equal(10.0, (totalDistance rides).Value)
...
let ``cancellationRate is notHappened over total`` () =
    ...
    let notHappened =
        [ mkNotHappened "client_cancelled"
          mkNotHappened "driver_rejected"
          mkNotHappened "client_did_not_show" ]
...
let ``countByState counts states across both sequences`` () =
    ...
    Assert.Equal(2, counts.["finished"])
    Assert.Equal(2, counts.["client_cancelled"])
...
let ``cashShare is cash earned over total earned`` () =
    let rides =
        [ mkRide [ "Kurs", 30m ] 1m (utc (2024, 1, 1, 10)) "cash" "default"
          mkRide [ "Kurs", 10m ] 1m (utc (2024, 1, 1, 11)) "in_app" "card" ]
...
let ``earnedByHourOfDay buckets by Warsaw-local hour (winter +1, summer +2)`` () =
    let rides =
        [ mkRide [ "Kurs", 10m ] 1m (utc (2024, 1, 15, 10)) "in_app" "card" // 10:30Z -> 11 local
          mkRide [ "Kurs", 20m ] 1m (utc (2024, 7, 15, 10)) "in_app" "card" ] // 10:30Z -> 12 local
...
let ``averageEarnedByHourOfDay averages daily sums per hour`` () =
    ...
    let rides =
        [ mkRide [ "Kurs", 4m ] 1m (utc (2024, 1, 15, 10)) "in_app" "card"
          mkRide [ "Kurs", 6m ] 1m (utc (2024, 1, 15, 10)) "in_app" "card"
          mkRide [ "Kurs", 30m ] 1m (utc (2024, 1, 16, 10)) "in_app" "card" ]
...
let ``temporal sequences are sorted by key`` () =
    let rides =
        [ mkRide [ "Kurs", 1m ] 1m (utc (2024, 1, 1, 20)) "in_app" "card"
          mkRide [ "Kurs", 1m ] 1m (utc (2024, 1, 1, 5)) "in_app" "card" ]
